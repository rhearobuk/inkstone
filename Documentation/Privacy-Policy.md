# Inkstone Privacy Policy

**Effective date:** September 16, 2026

Inkstone is an authoring application maintained by Robert Rhea. This policy
describes how the app handles data when used on macOS, iPadOS, and visionOS.

## Data stored by Inkstone

Inkstone stores the projects and information that a person enters or imports,
...
When iCloud is available and Inkstone is signed with its CloudKit capability,
...
store. Inkstone does not operate a developer-controlled account service, and the
...
Inkstone connects directly to the selected provider: OpenAI, Anthropic, Google,
...
that setting is not a promise of zero retention. Inkstone does not send content
...
API keys are stored in the system Keychain. They are not stored in Inkstone's
project database, UserDefaults, or iCloud key-value store.

## Data sharing, tracking, and sale

Inkstone does not include advertising, analytics, tracking, or a data broker
...
be cleared in Preferences. Deleting Inkstone from a device does not remove data
from other devices or iCloud; users should delete projects in Inkstone first.
...
[Inkstone support channels](https://github.com/rhearobuk/inkstone/blob/main/SUPPORT.md).
For security issues, follow the
[security reporting policy](https://github.com/rhearobuk/inkstone/blob/main/SECURITY.md).
